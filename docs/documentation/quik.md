...
注意：预授权属于消费类型的交易，默认是做DCC的，在调用接口之前一定要将刷卡回调中的汇率信息设置给request，确保预授权的DCC交易能够顺利进行。
...
当交易卡片为外卡时，消费类（消费、预授权）交易可选择进行DCC转EDC。  
